...
class OwnerIdMigration implements ServiceDbMigration {
  ...
  public async up(knex: Knex): Promise<void> {
    await knex.schema.alterTable("Repository", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "accountId");
    });

    await knex.schema.alterTable("Label", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "repositoryId");
    });

    await knex.schema.alterTable("IssueLike", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "repositoryId");
    });

    await knex.schema.alterTable("IssueLikeLabels", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "repositoryId");
    });

    await knex.schema.alterTable("Search", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "accountId");
    });
  }
}
