...
class OwnerIdMigration implements ServiceDbMigration {
  ...
  public async up(knex: Knex): Promise<void> {
    await knex.schema.alterTable("File", (table: Knex.CreateTableBuilder): void => {
    ...
    await knex.schema.alterTable("Thread", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "accountId");
    });

    await knex.schema.alterTable("ThreadLabel", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "accountId");
    });

    await knex.schema.alterTable("MailSearch", (table: Knex.CreateTableBuilder): void => {
      table.renameColumn("ownerId", "accountId");
    });
  }
}
