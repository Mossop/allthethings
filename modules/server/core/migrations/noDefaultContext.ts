...
import { id, insertFromTable, updateFromTable } from "#server/utils";
...
export async function up(knex: Knex): Promise<void> {
  ...
  let defaultContexts: string[] = await knex
    .from("Context")
    .where("name", "")
    .pluck("id");
  ...
  await knex.schema.alterTable(
    "Project",
    (table: Knex.CreateTableBuilder): void => {
      table.text("userId").notNullable().alter();

      table
        .foreign(["contextId", "userId"])
