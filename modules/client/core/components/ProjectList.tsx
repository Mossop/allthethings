...
const ProjectItem = ReactMemo(function ProjectItem({
  ...
  let classes = useStyles({ depth });
  let { isDropping, dropRef } = useDropTarget(project);

  let { isDragging, dragRef } = useDragSource(project);
    ...
    <>
      ...
      {project.subprojects.length > 0 && (
        <List className={classes.innerList}>
          {nameSorted(project.subprojects).map((child: Project) => (
            <ProjectItem
              key={child.id}
              project={child}
              taskList={taskList}
...
export default ReactMemo(function ProjectList(): ReactResult {
  ...
  let { isDropping: isInboxDropping, dropRef: inboxDropRef } = useDropTarget(
    user.inbox,
  );

  let { isDropping: isRootDropping, dropRef: rootDropRef } =
  ...
  return (
    <Paper elevation={2} component="nav" square={true} className={classes.nav}>
      <List component="div" className={classes.list}>
        ...
        <TreeItem
          ...
          icon={<Icons.Project />}
          selected={
            view.type == ViewType.TaskList && taskList?.id == context.id
          }
        ...
        {nameSorted(context.subprojects).map((project: Project) => (
          <ProjectItem
            key={project.id}
            project={project}
            taskList={taskList}
