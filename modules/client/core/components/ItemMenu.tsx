...
function renderEditDialog(item: Item, onClosed: () => void): ReactResult {
  ...
  if (isServiceItem(item))
    throw new Error("Cannot edit service items.");
...
export default ReactMemo(function ItemMenu({
  ...
  let isCurrentlyListed = isServiceItem(item) ? item.detail.isCurrentlyListed : false;
  ...
  return <>
    ...
    >
      ...
      {
        !isServiceItem(item) && <MenuItem onClick={openEditDialog}>
